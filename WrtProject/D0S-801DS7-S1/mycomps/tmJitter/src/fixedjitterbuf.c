...
{
	return zenCalloc(1, sizeof(struct fixed_jb_frame));
}
...
static void get_jb_head(struct fixed_jb *jb, struct fixed_jb_frame *frame)
{
	...
	memcpy(frame->data,fr->data,fr->ms);
...
///////////////////////////////////////////////////////////////////////////////
static struct fixed_jb * jbfixed;

void InitFixedJitter()
{
    
    struct fixed_jb_conf jbconf;
    jbconf.jbsize =20;
    jbconf.resync_threshold = 0;
    jbfixed  = fixed_jb_new(&jbconf);
}
void ExitFixedJitter()
{
  
    struct fixed_jb_frame  jbframe;
    
    while(1){
        jbframe.data = NULL;
        if(fixed_jb_remove(jbfixed,&jbframe) == FIXED_JB_NOFRAME)
            break;
        if(jbframe.data != NULL)
            zenFree(jbframe.data);
    }
    fixed_jb_destroy(jbfixed);
    jbfixed = NULL;
}
int  FixedJitter_Put(void* data ,long ms,long ts,long now)
{
   if( fixed_jb_put(jbfixed,data,ms,ts,now) == FIXED_JB_OK){
       return 1;
   }
   return 0;
}
int  FixedJitter_Get(void** data,long* buflen, long now)
{ 
    int ret;
    struct fixed_jb_frame frame;
    memset(&frame,0,sizeof(struct fixed_jb_frame));  
   
   ret = fixed_jb_get(jbfixed,&frame,now,20);
   switch(ret){
    case FIXED_JB_OK:
        *data = frame.data;
        *buflen = frame.ms;
        return 1;
    case FIXED_JB_NOFRAME:
        *data = NULL;
        return 0;
    case FIXED_JB_INTERP: //请插入一个空包
        return 2;        
    case FIXED_JB_DROP: //该buf请丢弃        
        *data = frame.data;
        *buflen = frame.ms;
        return 3;
   }
