...
void init_mempool(int size)
{
	...
	memHead->prev = NULL;
	memHead->next = NULL;
	printf("set memHead ...%x,%d\n",memHead,memHead->size);	       
}
...
void mempool_free(void* p)
{
        ...
        base->tag = TAG_FREE;
        // printf("free base %x,base->prev=%x,base->next=%x\n",base,base->prev,base->next);
        ...
        if (other && other->tag == TAG_FREE) {
                ...
                base = other;
              //  printf("1:free base %x,base->prev=%x,base->next=%x\n",base,base->prev,base->next);
        }
	// printf("2:free base %x,base->prev=%x,base->next=%x\n",base,base->prev,base->next);
	// printf("2:prev other  other %x,other->prev=%x,other->next=%x\n",other,other->prev,other->next);
        other = base->next;
        if (other && other->tag == TAG_FREE) {
                ...
                if (base->next)
                        base->next->prev = base;
  		//printf("3:free base %x,base->prev=%x,base->next=%x\n",base,base->prev,base->next);                        
        }
      // printf("4:free base %x,base->prev=%x,base->next=%x\n",base,base->prev,base->next);
     //  printf("4:next other  other %x,other->prev=%x,other->next=%x\n",other,other->prev,other->next);
      // other = other->next;
      // printf("5:next other  other %x,other->prev=%x,other->next=%x\n",other,other->prev,other->next);
        	...
        	/*
                for (base = memHead; base != NULL; base = base->next) {
                        if(base->tag == TAG_USED)
                                printf("malloc failed size = %d base = %x,base->size = %d \n",size,base,base->size);
                        if(base->tag == TAG_FREE){
                        	printf("ALL FREE malloc failed size = %d base = %x,base->size = %d \n",size,base,base->size);
                        }
                }
                */
