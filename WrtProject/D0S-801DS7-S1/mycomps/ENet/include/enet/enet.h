...
ENET_API int        enet_host_flush (ENetHost *);
 ...
 * enet 应用库 by ljw 
 */
typedef struct _tagAppENetCallbacks
{
	void (*AppRecvCallback)(unsigned long ip,short port ,void* buf,int len); //报警回调
	void (*AppRecvCallback2)(unsigned long ip,short port,void* buf,int len);//其他回调
}AppENetCallbacks;

//listenport=0;默认采用20302
//sendport=0默认采用20300
ENET_API int App_InitENet(short listenport,short sendport, AppENetCallbacks* callbacks);
ENET_API int App_UninitEnet();
ENET_API int App_Enet_Send(char* ip,short port,void* buf,int len);
ENET_API int App_Enet_Send2(unsigned long ip,short port,void* buf,int len);
