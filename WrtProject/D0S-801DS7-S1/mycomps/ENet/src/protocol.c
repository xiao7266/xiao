...
enet_protocol_handle_bandwidth_limit (ENetHost * host, ENetPeer * peer, const ENetProtocol * command)
{
    ...
    enet_uint8* pTempAddr =(enet_uint8*) &command -> bandwidthLimit.incomingBandwidth;
    memcpy(&incomingBandwidth,pTempAddr,4);
    memcpy(&outgoingBandwidth,pTempAddr+4,4);

    peer -> incomingBandwidth = ENET_NET_TO_HOST_32(incomingBandwidth);//ENET_NET_TO_HOST_32 (command -> bandwidthLimit.incomingBandwidth);
    peer -> outgoingBandwidth =ENET_NET_TO_HOST_32(outgoingBandwidth);// ENET_NET_TO_HOST_32 (command -> bandwidthLimit.outgoingBandwidth);
...
enet_protocol_handle_incoming_commands (ENetHost * host, ENetEvent * event)
{
    ...
    if (host -> receivedDataLength < sizeof (ENetProtocolHeader)){
	  printf("sizeof (ENetProtocolHeader) = %d \n",sizeof (ENetProtocolHeader));
      return 0;
	}
   ...
   printf("currentData = %x \n",currentData);
   printf("host -> receivedData = %d \n",host -> receivedData);
...
/** Sends any queued packets on the host specified to its designated peers.
    ...
    @ingroup host
*/
int enet_host_flush (ENetHost * host)
{
    host -> serviceTime = enet_time_get ();

    return enet_protocol_send_outgoing_commands (host, NULL, 0);
}
