...
int zenMemInit(int reserved)
{

        //  WRT_HAVE_DBG();
#ifdef USE_MEMPOOL
        printf("begin malloc mem pool %d\n",MEM_POOL_SIZE);
        init_mempool(MEM_POOL_SIZE);
#endif
...
extern void* zenDebugMalloc(int size,char* file,int line)
{
        ...
        if(ptr)
                ...
                printf("malloc failed %d,%s,%d\n",line,file,size);
        return  ptr;
}
#endif

void*zenCalloc(int num,int size)
{
...
void* zenRealloc(void* mem,int size){
