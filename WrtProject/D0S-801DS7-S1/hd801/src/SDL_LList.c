...
int SDL_LListInsert(SDL_LinkedList *list,SDL_LinkedListElem *newElem,SDL_LinkedListElem *oldElem){
        if(oldElem != NULL){
                SDL_LinkedListElem* pOld = oldElem->prev;
                if(pOld){
                        pOld->next = newElem;
                }	  	
                ...
                SDL_LListAddToEnd(list,newElem);
        }
        return 1;
